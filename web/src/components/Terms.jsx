...
export default function Terms({ onClose, onRead }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(30,36,48,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: "16px" }}>
      <div style={{ background: "#fff", borderRadius: 16, maxWidth: 600, width: "100%", maxHeight: "90vh", display: "flex", flexDirection: "column", boxShadow: "0 8px 40px rgba(0,0,0,0.25)" }}>

        {/* 고정 헤더 */}
        <div style={{ padding: "20px 24px 14px", borderBottom: "1px solid #e8eaf0", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <p style={{ fontWeight: 800, fontSize: 18, color: "#1a1f2e", margin: 0 }}>개인정보처리방침 및 이용약관</p>
            <button onClick={handleClose} style={{ background: "none", border: "none", fontSize: 22, color: "#6b7280", cursor: "pointer", padding: "0 4px", lineHeight: 1 }}>✕</button>
          </div>
          {!hasRead && (
            <p style={{ fontSize: 12, color: "#9ca3af", margin: "8px 0 0" }}>
              전체 내용을 확인하려면 아래로 스크롤해주세요.
            </p>
          )}
          {hasRead && (
            <p style={{ fontSize: 12, color: C.green, fontWeight: 700, margin: "8px 0 0" }}>
              ✓ 전체 내용을 확인하였습니다.
            </p>
          )}
        </div>
        ...
        >
          ...
          <Section title="제2조 (수집하는 개인정보 항목)">
            <Table rows={[
              ...
              ["추가 수집", "업무 노트(출근장소·퇴근장소·외근장소·오늘 업무), 랜덤 위치확인 결과"],
              ["선택 수집", "법인명, 팀, 직무, 전화번호"],
            ]} />
          </Section>
          ...
          <Section title="제3조 (개인정보 수집 및 이용 목적)">
            <ul style={{ paddingLeft: 18, margin: 0 }}>
              ...
              <li>근무지와의 GPS 거리 기록</li>
              <li>외출·복귀 기록 및 외부 근무 관리</li>
              <li>랜덤 위치 확인을 통한 재택·현장 근무 모니터링</li>
              <li>지각·출퇴근 누락 등 근태 사항 기록 및 월별 리포트 생성</li>
          ...
          <Section title="제5조 (위치정보 수집·이용 동의 — 위치정보보호법 제18조)">
            <div style={{ background: "#f8f9fc", borderRadius: 10, padding: "12px 14px", marginBottom: 10 }}>
              ...
              <Table rows={[
                ...
                ["이용 목적", "출퇴근 거리 기록, 외출 기록, 랜덤 위치 확인"],
        ...
        {/* 고정 푸터 */}
        <div style={{ padding: "14px 24px", borderTop: "1px solid #e8eaf0", flexShrink: 0 }}>
          <button
            onClick={handleClose}
            disabled={!hasRead}
            style={{
              width: "100%", padding: "13px",
              background: hasRead ? "#1a1f2e" : "#d1d5db",
          ...
          >
            {hasRead ? "확인" : "아래로 스크롤하여 전체 내용을 확인하세요 ↓"}
          </button>
        </div>
      </div>
    </div>
